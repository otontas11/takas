<template>
    <ion-page>
        ...
        <ion-content class="ion-padding">
            ...
            <ion-card v-for="item in filteredUserAdsList" :key="item.product?.product_code"
                style="border: 1px solid palevioletred;"
                :class="{ 'text-danger': !item.product?.status, 'text-success': item.product?.status }">

                <!-- Sol taraf: Ürün görseli -->
                <div class="card-left" style="margin-right: 12px; display: flex; height: 100%; align-items: center;">
                    <img @click="goProduct(item.product.product_code)"
                        :src="'https://takasimos3.s3.eu-north-1.amazonaws.com' + (item.product?.showcase_image.replace('240', '640'))"
                        style="width: 100%; height: 100%; border-radius: 8px; object-fit: cover;" />
                </div>


                <!-- Sağ taraf: Ürün bilgileri -->
                <div class="card-right" style="display: flex; flex-direction: column; gap: 1px; margin-top: 12px;">
                    ...
                    <!-- Durum kontrolü: Yayında değilse -->
                  <div v-if="!item.product?.status" style="display: flex; align-items: center; margin-bottom: 5px; height: 10px;">
                    <p
                        v-if="new Date(item.product?.due_date) < new Date()"
                        style="padding: 1px; border-radius: 5px; background-color: #901B6A; color: white; width: 80%; text-align: center;">
                      Yeniden yayınla
                    </p>
                    <p
                        v-else
                  ...
                  <!-- Doping bilgileri -->
                    <div v-for="doping in item.product?.doping" :key="doping.doping_code"
                        style="display: flex; margin-left: 10px; align-items: center; position: relative; height: 30px; margin-bottom: 0;">
                        <p style="padding-left: 8px; padding-right: 8px; padding-top: 2px; border-radius: 5px; background-color: #901B6A; color: white; width: auto; height: 20px;  position: relative; overflow: visible;"
                            :class="{ 'text-danger': !item.product?.status, 'text-success': item.product?.status }">
                    ...
                    <!-- <div v-if ="item.product?.status === false">
                        <p style="padding: 1px; border-radius: 5px; background-color: #901B6A; color: white; width: 100px; text-align: center;">
                            Onay Bekliyor
                        </p>
                    </div> -->

                    <!-- Öne Çıkar Butonu. v-if="item.product?.doping?.length === 0 && item.product?.status === true" -->
                    <ion-button v-if="item.product?.doping?.length === 0 && item.product?.status === true"
                        @click="buyDoping(item.product)" size="small" shape="round"
...
const loadUserData = () => {
    productApi.myProducts().then(res => {
                ...
                // Her ürün için dopingleri product objesinin içine ekliyoruz
                userAdsList.value.forEach((product: { product_code: any; product: any;[key: string]: any }) => {
                    product.product.doping = dopingList.filter((dopingItem: { product_code: any }) =>
    ...
    productApi.deleteProduct(item)
        ...
        .then(res => {
            ...
            productApi.myProducts().then(res => {
                ...
                console.log("Güncellenmiş userAdsList:", userAdsList.value);
            });
        })
        .catch(error => {
